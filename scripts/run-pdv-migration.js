/**
 * Execute PDV migration via api_dinamico
 * Splits the migration into individual statements and runs them sequentially
 */
...
const statements = [
  ...
  // ── 9. Add CHECK constraint for item_kind on services (separate from column creation) ──
  // Skipped — CHECK with ADD COLUMN IF NOT EXISTS doesn't work well;
  // the item_kind column will accept any text but app validates at service layer.

  // ── Self-composition constraint ──
...
async function run() {
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    try {
      await axios.post(API, { sql }, { timeout: 30000 });
      ok++;
      process.stdout.write(`✅ [${i + 1}/${statements.length}] ${label}...\n`);
    } catch (e) {
      const msg =
        typeof e.response?.data === "string"
          ? e.response.data.substring(0, 200)
          : e.message;
      ...
      if (
        ...
        process.stdout.write(
          `⚠️  [${i + 1}/${statements.length}] ${label}... (already exists, OK)\n`,
        );
      } else {
        fail++;
        errors.push({ index: i + 1, label, msg });
        process.stdout.write(
          `❌ [${i + 1}/${statements.length}] ${label}... FAILED: ${msg}\n`,
        );
      }
    }
  }
  ...
  if (errors.length > 0) {
    console.log("\nFailed statements:");
    errors.forEach((e) => console.log(`  [${e.index}] ${e.label}: ${e.msg}`));
  }
}
