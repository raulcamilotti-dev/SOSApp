...
const listRowsForTenant = async (tenantId?: string | null): Promise<Row[]> => {
  ...
  const response = await api.post(CRUD_ENDPOINT, {
    ...
    ...buildSearchParams(filters, { sortColumn: "table_name ASC" }),
  });
...
export default function AgentPlaybookTablesScreen() {
  const { user } = useAuth();
  ...
  const fields: CrudFieldConfig<Row>[] = [
    ...
    { key: "is_required", label: "Obrigatória", visibleInList: true },
    {
      key: "purpose",
      label: "Finalidade",
      type: "multiline",
  ...
  return (
    <CrudScreen<Row>
      ...
      searchFields={["table_name", "purpose", "access_mode"]}
      fields={fields}
      loadItems={loadItems}
      ...
      getTitle={(item) => String(item.table_name ?? "Tabela")}
      getDetails={(item) => [
        { label: "Acesso", value: String(item.access_mode ?? "-") },
        { label: "Obrigatória", value: String(item.is_required ?? "-") },
