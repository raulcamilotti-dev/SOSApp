...
// src/db.ts
async function executeQuery(env2, query, params = []) {
...
function buildWhere(body, startIndex = 1) {
  ...
  const where = whereParts.length ? "WHERE " + whereParts.join(" " + combineType + " ") : "";
  return { where, params, paramIndex };
}
__name(buildWhere, "buildWhere");
function buildList(body) {
  const table3 = body.table;
  if (!table3) throw new Error("table is required");
  ...
  if (body.sort_column) {
    const sortParts = body.sort_column.split(",").map((s) => {
      ...
      const col = parts[0];
      const dir3 = (parts[1] || "").toUpperCase();
      if (col !== "*") validateIdentifier(col);
      return dir3 === "DESC" ? '"' + col + '" DESC' : '"' + col + '" ASC';
    });
  ...
  const query = "SELECT " + selectClause + ' FROM "' + table3 + '" ' + where + " " + orderBy + " " + limitClause + " " + offsetClause;
...
function buildCreate(body) {
  ...
  const placeholders = keys.map((k, i) => {
    params.push(payload[k] !== void 0 ? payload[k] : null);
    return "$" + (i + 1);
  });
  const query = 'INSERT INTO "' + table3 + '" (' + columns + ") VALUES (" + placeholders.join(", ") + ") RETURNING *";
  return { query, params };
}
__name(buildCreate, "buildCreate");
function buildUpdate(body) {
  ...
  const matchColumn = table3 === "controle_atendimento" ? "session_id" : "id";
  ...
  const query = 'UPDATE "' + table3 + '" SET ' + setParts.join(", ") + ' WHERE "' + matchColumn + '" = $' + paramIndex + " RETURNING *";
  return { query, params };
}
__name(buildUpdate, "buildUpdate");
function buildDelete(body) {
  const table3 = body.table;
  if (!table3) throw new Error("table is required");
  validateIdentifier(table3);
  const payload = body.payload ?? {};
  ...
  const deletedAt = payload.deleted_at || (/* @__PURE__ */ new Date()).toISOString();
  const query = 'UPDATE "' + table3 + '" SET "deleted_at" = $1 WHERE "id" = $2 RETURNING *';
  const params = [deletedAt, id];
  return { query, params };
}
__name(buildDelete, "buildDelete");
function buildCount(body) {
  const table3 = body.table;
  if (!table3) throw new Error("table is required");
  validateIdentifier(table3);
  const { where, params } = buildWhere(body);
  const query = 'SELECT COUNT(*)::int AS count FROM "' + table3 + '" ' + where;
  return { query: query.trim(), params };
}
__name(buildCount, "buildCount");
var ALLOWED_FUNCTIONS = /* @__PURE__ */ new Set(["SUM", "COUNT", "AVG", "MIN", "MAX"]);
function buildAggregate(body) {
  const table3 = body.table;
  ...
  for (const agg of aggregates) {
    ...
    const alias = agg.alias ? validateIdentifier(agg.alias) : validateIdentifier(fn.toLowerCase() + "_" + agg.field);
  ...
  const groupByClause = groupBy.length ? "GROUP BY " + groupBy.map(validateIdentifier).join(", ") : "";
  let orderByClause = "";
  if (body.sort_column) {
    const sortParts = body.sort_column.split(",").map((s) => {
      const parts = s.trim().split(/\s+/);
...
__name(buildAggregate, "buildAggregate");
function buildBatchCreate(body) {
  const table3 = body.table;
  ...
  const valueRows = items.map((item) => {
    const placeholders = keys.map((k) => {
      params.push(item[k] !== void 0 ? item[k] : null);
      return "$" + paramIndex++;
    });
    return "(" + placeholders.join(", ") + ")";
  });
  const query = 'INSERT INTO "' + table3 + '" (' + columns + ") VALUES " + valueRows.join(", ") + " RETURNING *";
  return { query, params };
}
__name(buildBatchCreate, "buildBatchCreate");
...
function corsResponse(status, body, extraHeaders) {
  ...
  const responseBody = Array.isArray(body) && body.length === 0 ? "" : JSON.stringify(body);
...
function authenticate(request, env2) {
  ...
  return apiKey === env2.API_KEY;
}
__name(authenticate, "authenticate");
async function handleCrud(body, env2) {
  ...
  try {
    ...
    const rows = await executeQuery(env2, queryResult.query, queryResult.params);
...
async function handleDinamico(body, env2) {
  const sql = body.sql;
  if (!sql || typeof sql !== "string") {
    return errorResponse(400, "sql is required");
  }
  try {
    const rows = await executeQuery(env2, sql);
    return corsResponse(200, rows);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
...
__name(handleDinamico, "handleDinamico");
async function handleTablesInfo(body, env2) {
  ...
  `;
  try {
    const rows = await executeQuery(env2, query, [tableName]);
    return corsResponse(200, rows);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return errorResponse(400, message);
...
__name(handleTablesInfo, "handleTablesInfo");
async function handleTables(env2) {
  ...
  `;
  try {
    const rows = await executeQuery(env2, query);
    return corsResponse(200, rows);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return errorResponse(400, message);
...
async function handleHealth(env2) {
  try {
    const rows = await executeQuery(env2, "SELECT 1 AS ok");
...
var index_default = {
  async fetch(request, env2) {
    ...
    if (!authenticate(request, env2)) {
      return errorResponse(401, "Unauthorized");
    }
    try {
      if (request.method === "POST" && (path === "/api_crud" || path === "/webhook/api_crud")) {
        const body = await request.json();
        return handleCrud(body, env2);
      }
      if (request.method === "POST" && (path === "/api_dinamico" || path === "/webhook/api_dinamico")) {
        const body = await request.json();
        return handleDinamico(body, env2);
      }
      if (request.method === "POST" && (path === "/tables_info" || path === "/webhook/tables_info")) {
        const body = await request.json();
        return handleTablesInfo(body, env2);
      }
      if (request.method === "GET" && (path === "/tables" || path === "/webhook/tables")) {
        return handleTables(env2);
      }
